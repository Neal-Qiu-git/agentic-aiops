...
class AIOpsEngine:
    ...
    def _init_tools(self):
        ...
        # --- P1 补全 (v4.5.0) ---
        try:
            from ..tools.container_runtime_tools import ContainerdTool, PodmanTool
            for t in [ContainerdTool(), PodmanTool()]:
                self.tool_registry.register(t); registered += 1
        except ImportError as e:
            logger.warning(f"容器运行时工具加载失败: {e}")

        try:
            from ..tools.webserver_tools import WildFlyTool, CaddyTool
            for t in [WildFlyTool(), CaddyTool()]:
                self.tool_registry.register(t); registered += 1
        except ImportError as e:
            logger.warning(f"Web 服务器工具加载失败: {e}")

        try:
            from ..tools.profiling_tools import MimirQuery, PyroscopeProfiles, PyroscopeMetrics
            for t in [MimirQuery(), PyroscopeProfiles(), PyroscopeMetrics()]:
                self.tool_registry.register(t); registered += 1
        except ImportError as e:
            logger.warning(f"性能分析工具加载失败: {e}")

        try:
            from ..tools.cloud_ext_tools import DynamoDBTool, GCPCost
            for t in [DynamoDBTool(), GCPCost()]:
                self.tool_registry.register(t); registered += 1
        except ImportError as e:
            logger.warning(f"云扩展工具加载失败: {e}")
