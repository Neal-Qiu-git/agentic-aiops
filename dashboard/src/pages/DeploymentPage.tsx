...
export default function DeploymentPage() {
  ...
  const loadAll = () => {
    fetchDeploymentSummary().then(r => setSummary(r.data));
    fetchNodes().then(r => setNodes(r.data));
    fetchPods().then(r => setPods(r.data));
    fetchDeployments().then(r => setDeployments(r.data));
    fetchEvents().then(r => setEvents(r.data));
  };
                ...
                <tbody>
                  {nodes.map((n, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                      ...
                      <td style={{ width: 120 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ flex: 1, height: 6, background: '#2a3050', borderRadius: 3 }}><div style={{ height: '100%', width: `${n.cpu}%`, background: parseInt(n.cpu) > 70 ? '#ef4444' : '#3b82f6', borderRadius: 3 }} /></div><span style={{ fontSize: 11, color: '#94a3b8', minWidth: 30 }}>{n.cpu}%</span></div></td>
                      <td style={{ width: 120 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><div style={{ flex: 1, height: 6, background: '#2a3050', borderRadius: 3 }}><div style={{ height: '100%', width: `${n.memory}%`, background: parseInt(n.memory) > 80 ? '#ef4444' : '#8b5cf6', borderRadius: 3 }} /></div><span style={{ fontSize: 11, color: '#94a3b8', minWidth: 30 }}>{n.memory}%</span></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ background: '#1a1f35', border: '1px solid #2a3050', borderRadius: 12, padding: 20, marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
              {deployments.map((d, i) => {
                ...
                const [ready, desired] = (d.ready || '0/0').split('/').map(Number);
                return (
                  <div key={i} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10, padding: 16, borderLeft: `3px solid ${cfg.color}` }}>
                    ...
                    <div style={{ fontSize: 11, color: '#94a3b8', marginBottom: 6 }}>NS: {d.ns || d.namespace} · Strategy: {d.strategy || '-'}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: ready === desired ? '#10b981' : '#f59e0b' }}>{d.ready || d.replicas}</span>
