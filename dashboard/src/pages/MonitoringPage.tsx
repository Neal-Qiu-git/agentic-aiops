...
export default function MonitoringPage() {
  ...
  useEffect(() => {
    detectMode().then(real => {
      ...
      fetchMonitoringSummary().then(r => setSummary(r.data));
      fetchAlerts().then(r => setAlerts(r.data.alerts ?? []));
      fetchTargets().then(r => setTargets(r.data.targets ?? []));
    });
  }, []);
  ...
  const s = summary || { cpu_usage: 0, memory_usage: 0, disk_usage: 0 };
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
        {[
          { label: 'CPU 使用率', value: `${(s.cpu_usage || 0).toFixed(1)}%`, icon: '🖥️', color: '#3b82f6', bg: 'rgba(59,130,246,0.12)' },
          { label: '内存使用率', value: `${(s.memory_usage || 0).toFixed(1)}%`, icon: '💾', color: '#8b5cf6', bg: 'rgba(139,92,246,0.12)' },
          { label: '磁盘使用率', value: `${(s.disk_usage || 0).toFixed(1)}%`, icon: '📀', color: '#06b6d4', bg: 'rgba(6,182,212,0.12)' },
          { label: '活跃告警', value: String(alerts.length), icon: '🚨', color: '#ef4444', bg: 'rgba(239,68,68,0.12)' },
        ].map((item, i) => (
        ...
        <div style={{ background: '#1a1f35', border: '1px solid #2a3050', borderRadius: 12, padding: 20 }}>
          ...
          {alerts.map((a, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, marginBottom: 8, borderLeft: `3px solid ${statusColors[a.severity] || '#64748b'}` }}>
              ...
              <div style={{ fontSize: 10, color: '#64748b' }}>{a.started_at}</div>
            </div>
          ))}
        </div>
        <div style={{ background: '#1a1f35', border: '1px solid #2a3050', borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: 16 }}>
            <div style={{ textAlign: 'center' }}><div style={{ fontSize: 28, fontWeight: 700, color: '#10b981' }}>{targets.filter(t => t.status === 'up').length}</div><div style={{ fontSize: 11, color: '#64748b' }}>在线</div></div>
            <div style={{ textAlign: 'center' }}><div style={{ fontSize: 28, fontWeight: 700, color: '#ef4444' }}>{targets.filter(t => t.status !== 'up').length}</div><div style={{ fontSize: 11, color: '#64748b' }}>离线</div></div>
          </div>
          {targets.map((t, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 0', borderBottom: i < targets.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', fontSize: 11 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: healthColors[t.status] || '#64748b' }} />
              <span style={{ color: '#e2e8f0', flex: 1, fontFamily: 'monospace' }}>{t.instance}</span>
              <span style={{ color: '#64748b' }}>{t.last_scrape}</span>
            </div>
          ))}
        </div>
      </div>
